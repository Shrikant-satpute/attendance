			...
			<div class="panel-body text-center">
				  <?php
					  ...
					  for($i = 1; $i < 8; $i++) {
						  ...
						  $query_subjectPending = "SELECT subject.sname, subject.id from subject INNER JOIN user_subject WHERE user_subject.id = subject.id AND user_subject.uid = {$_SESSION['uid']}  ORDER BY subject.sname";
						  ...
						  if(($weekday!="saturday") && ($weekday!="sunday")) {
							  for($j = 0; $j<$noOfSubjectPending; $j++) {
								  ...
								  $sqlPending = "SELECT sid, isabsent FROM daily WHERE id=$subIdP AND date=$queryTimeStamp AND uid=$userId";
								  $stmtP = $conn->prepare($sqlPending); 
								  $stmtP->execute();
								  $resultP = $stmtP->fetchAll(PDO::FETCH_ASSOC); 
								  if(!empty($resultP)){
									  print "<p><a href='index.php?subject=" . $subIdP . "&date=" . $dateCurrentYMD ."' style='text-decoration:none;'>Class: <strong style='color:#555;'>" . $rsubPending[$j]['sname'] ."</strong> of <strong>" . $dateCurrent ."</strong></a> <span class='label label-success' style='border-radius:0%;'>Attendance Recorded</span> </p>";

								  }
								  else {
									  print "<p><a href='index.php?subject=" . $subIdP . "&date=" . $dateCurrentYMD ."' style='text-decoration:none;'>Class: <strong style='color:#555;'>" . $rsubPending[$j]['sname']."</strong> of <strong>" . $dateCurrent ."</strong></a> <span class='label label-danger' style='border-radius:0%;'>Mark Attendance Now!</span></p>";
								  }
							  }
			...
			<div class="panel-body text-center">
				  <?php
					  ...
					  for($i = 0; $i<$noOfSubject; $i++) {
						  $subId = $rsub[$i]['id'];
						  $sql = "SELECT sid, isabsent FROM daily WHERE id=$subId AND date=$todayDBQuery AND uid=$userId";
						  $stmt = $conn->prepare($sql); 
						  $stmt->execute();
						  $result = $stmt->fetchAll(PDO::FETCH_ASSOC); 
						  if(!empty($result)){
							  print "<p><a href='index.php?subject=" . $subId . "&date=" . $todayQuery ."' style='text-decoration:none;'>Class: <strong style='color:#555;'>" . $rsub[$i]['sname'] ."</strong> of <strong>Today's</strong> (" . $today .")</a> <span class='label label-success' style='border-radius:0%;'>Attendance Recorded</span> </p>";
						  }
						  else {
							  print "<p><a href='index.php?subject=" . $subId . "&date=" . $todayQuery ."' style='text-decoration:none;'>Class: <strong style='color:#555;'>" . $rsub[$i]['sname'] ."</strong> of <strong>Today's</strong> (" . $today .")</a> <span class='label label-danger' style='border-radius:0%;'>Mark Attendance Now!</span></p>";
						  }
					  }
				  ?>
			</div>
